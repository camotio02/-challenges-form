...
const validacao = () => {
    ...
    const newDate = fields?.date.split("-");
    const  birth= {
    ...
    console.log(1 > new Number(newDate[2]) < 31)
    return !true
    // CRIANDO REGRAS DE CADA CAMPO DO FORMULÁRIO
    const rulesfieldsForm = {
        name: { rule: fields?.nome.indexOf(" ") !== -1, message: '' },
        email: {
            rule: fields?.email.includes('@')
                && fields?.email.includes('.com'), message: ''
        },
        number: {
            rule: codigosDDD.includes(parseInt(fields?.number.substring(0, 2)))
                && fields?.number.length === 11,
            message: ''
        },
